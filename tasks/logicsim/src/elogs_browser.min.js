(function () {
    ...
    1: [function (require, module, exports) {
        (function (global) {
            ...
            const PublicConst = {MachineTruthTable: Const.MachineTruthTable, Port: Const.Port, Value: Const.Value};
            global.ELoGS = {circuit: Circuit, constants: PublicConst}
        }).call(this, typeof global !== "undefined" ? global : typeof self !== "undefined" ? self : typeof window !== "undefined" ? window : {})
    }, {"./modules/circuit": 2, "./modules/const": 4}], 2: [function (require, module, exports) {
        const stateProvider = require("./state");
        const Connection = require("./connection");
        const {SimpleMachine, AbstractMachine} = require("./machine");
        ...
        class Circuit {
            ...
            _getConnection(outMachineId, outPort, inMachineId, inPort) {
                ...
                let connectionToInput = this._getConnectionToInput(this.connectionGraph[outMachine.getId()].outputs, inMachine, inPort);
                let connectionToOutput = this._getConnectionToOutput(this.connectionGraph[inMachine.getId()].inputs, outMachine, outPort);
                let connection = null;
                if (connectionToInput !== null && connectionToOutput !== null) {
                    connection = new Connection(connectionToOutput.getOutMachine(), connectionToOutput.getOutPort(), connectionToInput.getInMachine(), connectionToInput.getInPort())
                }
                return connection
            }
            ...
            _getConnectionToInput(outputs, inMachine, inPort) {
                for (const connectionToIn of outputs) {
                    if (connectionToIn.getInMachine() === inMachine && connectionToIn.getInPort() === inPort) {
            ...
            _getConnectionToOutput(inputs, outMachine, outPort) {
                for (const connectionToOut of inputs) {
                    if (connectionToOut.getOutMachine() === outMachine && connectionToOut.getOutPort() === outPort) {
            ...
            _simulateMachine(machine) {
                ...
                } else {/*inputs=inputs.map(input=>input===Const.Value.UNDEFINED?Const.Value.LOW:input);*/
            ...
            _getFinalState() {
                return this.stateHistory.isEmpty() ? this.currentState.clone() : this.stateHistory.getLastState().clone()
            }

            _findAllMachineInputs(machine, searchTriggeringMachines = []) {/*if(searchTriggeringMachines.includes(String(machine))){return Const.Value.UNDEFINED}searchTriggeringMachines.push(String(machine));*/
                const inputs = [];
            ...
            _findInputMachines() {
                return Object.keys(this._getMachines()).filter(machineId => this._getMachineById(machineId).getInputDimension() === 0)
            }
    ...
    }, {"./connection": 3, "./const": 4, "./machine": 5, "./state": 6}], 3: [function (require, module, exports) {
        const Const = require("./const").constants;
        ...
        const ErrorMessage = {
            ...
            CONNECTION_DOESNT_EXIST: (outMachineId, outPort, inMachineId, inPort) => `Connection from machine ${outMachineId}, port ${outPort} to machine ${inMachineId}, port ${inPort} doesn't exist`,
            NON_EXISTENT_MACHINE: machineId => `Machine doesn't exist: ${machineId}`,
            PORT_OUT_OF_RANGE: (isOutPort, machineId, port, machinePortDimension) => `${isOutPort ? "Output" : "Input"} port ${port} is out of range in machine ${machineId}. Number of ${isOutPort ? "Output" : "Input"} ports in machine: ${machinePortDimension}.`,
        ...
        const EMPTY_INPUTS = [];
        const Value = {LOW: false, HIGH: true, UNDEFINED: undefined, DISCONNECTED: null};
        ...
        module.exports.constants = {
            ...
            Value: Value,
            MachineTruthTable: MachineTruthTable,
        ...
        const truthTableValidator = {
            validate: truthTable => {
                ...
                return truthTableValidator._validateInputs(Object.keys(truthTable)) && truthTableValidator._validateOutputs(Object.values(truthTable))
                ...
                let isValid = false;
                for (const valueKey in Const.Value) {
        ...
        class SimpleMachine extends AbstractMachine {
            ...
            calculateOutput(inputs) {
                ...
                inputs = inputs.join("");
                let res = this.truthTable[inputs];
                if (res !== undefined) {
                    return res
            ...
            getOutputOnPort(inputs, port) {
                let res = this.calculateOutput(inputs);
                if (res !== undefined) return res[port]
            }

            getOutputDimension() {
                return Object.keys(this.truthTable).length > 0 ? this.truthTable[Object.keys(this.truthTable)[0]].length : 0
            }

            getInputDimension() {
                return Object.keys(this.truthTable).length > 0 ? Object.keys(this.truthTable)[0].length : 0
            }
        ...
        module.exports = {SimpleMachine, AbstractMachine}
        ...
        module.exports = {State: State, StateHistory: StateHistory}
    }, {"./const": 4}]
}, {}, [1]);